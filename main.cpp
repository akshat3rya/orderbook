...
// Commands:
//   insert <id> <B|S> <price> <qty>   raw insert, no matching (build a book state by hand)
//   limit  <id> <B|S> <price> <qty>   submit through the matching engine
//   market <id> <B|S> <qty>           submit through the matching engine, no resting
//   cancel <id>
//   modify <id> <newQty>
//   book [depth]
//   help
...
static void printHelp() {
    std::cout << "insert <id> <B|S> <price> <qty>   add a resting order directly, no matching\n"
...
int main() {
    ...
    while (true) {
        ...
        if (cmd == "quit" || cmd == "exit") {
            ...
            if (!(iss >> id >> newQty)) {
                std::cout << "usage: modify <id> <newQty>\n";
                continue;
            }
            switch (book.modify(id, newQty)) {
                ...
                case ModifyResult::QTY_MUST_DECREASE:
                    std::cout << "error: new qty must be less than current qty\n";
                    break;
                case ModifyResult::RESULTED_IN_CANCEL:
                    std::cout << "ok (qty <= 0, order cancelled)\n";
                    break;
            }
        } else if (cmd == "book") {
            int depth = 10;
